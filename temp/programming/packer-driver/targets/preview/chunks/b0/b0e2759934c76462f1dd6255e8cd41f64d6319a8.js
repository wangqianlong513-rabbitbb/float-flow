System.register(["__unresolved_0", "cc", "__unresolved_1"], function (_export, _context) {
  ...
  function rotateTile(tile) {
    var config = getBaseTileConfig(tile.type);
  ...
  function resolveTileConfig(tile) {
    var base = getBaseTileConfig(tile.type);
    var routing = {};

    for (var key of Object.keys(base.routing)) {
      var out = base.routing[key];

      if (!out) {
        continue;
      }

      var rotatedIn = (_crd && rotateDirection === void 0 ? (_reportPossibleCrUseOfrotateDirection({
        error: Error()
