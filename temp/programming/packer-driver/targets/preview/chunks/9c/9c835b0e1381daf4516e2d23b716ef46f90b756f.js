      ...
      _export("SceneBootstrap", SceneBootstrap = (_dec = ccclass('SceneBootstrap'), _dec(_class = class SceneBootstrap extends Component {
        ...
        applyTheme(themeIdx) {
          ...
          if (this.homePageRoot) {
            var home = this.homePageRoot.getComponent(_crd && HomeRoot === void 0 ? (_reportPossibleCrUseOfHomeRoot({
              error: Error()
            }), HomeRoot) : HomeRoot);
          ...
          if (this.levelSelectRootNode) {
            var levelSelect = this.levelSelectRootNode.getComponent(_crd && LevelSelect === void 0 ? (_reportPossibleCrUseOfLevelSelect({
              error: Error()
          ...
          if (this.gamePageRoot) {
            var game = this.gamePageRoot.getComponent(_crd && GameRoot === void 0 ? (_reportPossibleCrUseOfGameRoot({
              error: Error()
            }), GameRoot) : GameRoot);
          ...
          if (this.settingsModalRootNode) {
            var settings = this.settingsModalRootNode.getComponent(_crd && SettingsModal === void 0 ? (_reportPossibleCrUseOfSettingsModal({
              error: Error()
        ...
        drawBackgroundDecorations(graphics, themeIdx) {
          ...
          for (var s of shards) {
            var halfW = s.size;
            var halfH = s.size * 0.5;
            var topCol = this.hex(s.colTop);
            var leftCol = this.hex(s.colLeft);
            var rightCol = this.hex(s.colRight);
          ...
          for (var i = 0; i < 45; i++) {
            var x = Math.sin(i * 997) * 620;
            var y = Math.cos(i * 631) * 360;
            var radius = 1.2 + i % 3 * 0.8;
        ...
        onLoad() {
          ...
          var bgRoot = this.createRoot('BgRoot', new Vec3(0, 0, 0));
          this.createBackground(bgRoot); // 1. Home Page Root (Default Active!)

          var homePage = this.createRoot('HomePage', new Vec3(0, 0, 0));
          this.homePageRoot = homePage;
          var home = homePage.addComponent(_crd && HomeRoot === void 0 ? (_reportPossibleCrUseOfHomeRoot({
            error: Error()
          }), HomeRoot) : HomeRoot); // 2. Game Page Root (Hidden by default until player starts a game!)

          var gamePage = this.createRoot('GamePage', new Vec3(0, 0, 0));
          this.gamePageRoot = gamePage;
          gamePage.active = false;
          var boardRoot = this.createRoot('BoardRoot', new Vec3(0, -75, 0), gamePage);
          var previewRoot = this.createRoot('PreviewRoot', new Vec3(0, -75, 0), gamePage);
          var runnerRoot = this.createRoot('RunnerRoot', new Vec3(0, -75, 0), gamePage);
          var cardRoot = this.createRoot('CardRoot', new Vec3(0, -215, 0), gamePage);
          var uiRoot = this.createRoot('UI', new Vec3(0, 0, 0), gamePage); // 1. Top Navigation Bar (matching Concept Art: Pause ||, Level Title, Progress Dots, Energy Pill)
          ...
          var levelLabel = this.createLabel(uiRoot, 'LevelLabel', new Vec3(-50, 316, 0), '旅途模式 · 5-12', 24, '#FFFFFF', 300, 50); // Tip Pill with glassmorphic background

          var tipPill = this.createRoot('TipPill', new Vec3(0, -120, 0), uiRoot);
          this.ensureTransform(tipPill, 340, 32);
          var tg = tipPill.addComponent(Graphics);
          ...
          tg.lineWidth = 1.5;
          tg.stroke();
          var tipLabel = this.createLabel(tipPill, 'TipLabel', new Vec3(0, 1, 0), '长按任意手牌：时空减速 ❓', 15, '#00F0FF', 320, 30);
          var statusLabel = this.createLabel(uiRoot, 'StatusLabel', new Vec3(0, -152, 0), '拖动手牌到地图放置', 17, '#60A5FA', 680, 30);
          var game = (_gamePage$getComponen = gamePage.getComponent(_crd && GameRoot === void 0 ? (_reportPossibleCrUseOfGameRoot({
            error: Error()
          ...
          var settingsModalRoot = this.createRoot('SettingsModalRoot', new Vec3(0, 0, 0));
          settingsModalRoot.active = false;
          this.settingsModalRootNode = settingsModalRoot;
          var settingsModal = settingsModalRoot.addComponent(_crd && SettingsModal === void 0 ? (_reportPossibleCrUseOfSettingsModal({
            error: Error()
          }), SettingsModal) : SettingsModal);
          ...
          var reviveModalRoot = this.createRoot('ReviveModalRoot', new Vec3(0, 0, 0));
          reviveModalRoot.active = false;
          this.reviveModalRootNode = reviveModalRoot;
          var reviveModal = reviveModalRoot.addComponent(_crd && ReviveModal === void 0 ? (_reportPossibleCrUseOfReviveModal({
            error: Error()
          ...
          var levelSelectRoot = this.createRoot('LevelSelectRoot', new Vec3(0, 0, 0));
          levelSelectRoot.active = false;
          this.levelSelectRootNode = levelSelectRoot;
          var levelSelect = levelSelectRoot.addComponent(_crd && LevelSelect === void 0 ? (_reportPossibleCrUseOfLevelSelect({
            error: Error()
          }), LevelSelect) : LevelSelect);
          ...
          var victoryPosterRoot = this.createRoot('VictoryPosterRoot', new Vec3(0, 0, 0));
          victoryPosterRoot.active = false;
          this.victoryPosterRootNode = victoryPosterRoot;
          var victoryPoster = victoryPosterRoot.addComponent(_crd && VictoryPoster === void 0 ? (_reportPossibleCrUseOfVictoryPoster({
            error: Error()
        ...
        createTopNav(parent, position) {
          var node = new Node('TopNav');
          node.layer = Layers.Enum.UI_2D;
          node.setParent(parent);
          node.setPosition(position);
          this.ensureTransform(node, 710, 76);
          var graphics = node.addComponent(Graphics); // Glassmorphic top navigation panel
          ...
          var homeReturnBtn = new Node('HomeReturnBtn');
        ...
        createBottomDeck(parent, position) {
          var node = new Node('BottomDeck');
          node.layer = Layers.Enum.UI_2D;
          node.setParent(parent);
          node.setPosition(position);
          this.ensureTransform(node, 700, 160);
          var graphics = node.addComponent(Graphics); // Luminous Sapphire Glassmorphic Tray (透亮深蓝宝石毛玻璃底台，告别灰色长方形！)
        ...
        createBulletTimeMeter(parent, position) {
          var node = new Node('BulletTimeMeter');
          node.layer = Layers.Enum.UI_2D;
          node.setParent(parent);
          node.setPosition(position);
          this.ensureTransform(node, 60, 150);
          var g = node.addComponent(Graphics); // Track
        ...
        createSideActionButtons(parent, game) {
          ...
          var eraseBtn = new Node('EraseBtn');
          ...
          var eg = eraseBtn.addComponent(Graphics);
        ...
        createCircularButton(parent, name, position, icon, labelText, onClick) {
          ...
          graphics.lineWidth = 2;
          graphics.stroke();
          var iconLabel = this.createLabel(node, 'Icon', new Vec3(0, 10, 1), icon, 24, '#FFFFFF', 50, 50);
          var textLabel = this.createLabel(node, 'Text', new Vec3(0, -28, 1), labelText, 14, '#60A5FA', 64, 24);
        ...
        createHeroButton(parent, name, position, text, onClick) {
          ...
          var graphics = node.addComponent(Graphics); // Vibrant Neon Cyan/Blue Hero Button
        ...
        createSecondaryButton(parent, name, position, text, onClick) {
          ...
          var height = 54;
          this.ensureTransform(node, width, height);
          var graphics = node.addComponent(Graphics); // Sleek Cyber Violet Secondary Button
        ...
        createLabel(parent, name, position, text, size, color, width, height) {
          if (width === void 0) {
            width = 520;
          }
