    ...
    execute: function () {
      ...
      _export("SettingsModal", SettingsModal = (_dec = ccclass('SettingsModal'), _dec(_class = class SettingsModal extends Component {
        ...
        updateDialogStyle() {
          ...
          var dg = this.dialogNode.getComponent(Graphics);
          if (!dg) dg = this.dialogNode.addComponent(Graphics);
          dg.clear();
          var isRose = this.selectedTheme === 1;
          var isGold = this.selectedTheme === 2;
        ...
        createHeader(parent) {
          ...
          var closeBtn = this.createNode('CloseBtn', new Vec3(300, 225, 0), parent);
        ...
        renderTabs() {
          ...
          tabs.forEach((name, idx) => {
            var isSelected = this.activeTab === idx;
            var tabNode = this.createNode("Tab_" + idx, new Vec3(tabXs[idx], 0, 0), this.tabsRoot);
        ...
        renderThemeTab(parent) {
          ...
          var cardXs = [-200, 0, 200];
          themes.forEach((th, idx) => {
            var isSelected = this.selectedTheme === idx;
            var card = this.createNode("Theme_" + idx, new Vec3(cardXs[idx], 40, 0), parent);
            ...
            if (isSelected) {
              var badge = this.createNode('Badge', new Vec3(65, 50, 0), card);
          ...
          var btRow = this.createNode('BTRow', new Vec3(0, -65, 0), parent);
          this.ensureTransform(btRow, 580, 50);
          var btG = btRow.addComponent(Graphics);
          ...
          var speeds = ['慢速 (25%)', '适中 (50%)', '极速 (100%)'];
          var speedXs = [-50, 80, 210];
          speeds.forEach((sp, i) => {
            var isSel = this.bulletTimeSpeed === i;
            var btn = this.createNode("Speed_" + i, new Vec3(speedXs[i], 0, 0), btRow);
            this.ensureTransform(btn, 116, 36);
            var bg = btn.addComponent(Graphics);
          ...
          var qRow = this.createNode('QRow', new Vec3(0, -135, 0), parent);
          this.ensureTransform(qRow, 580, 50);
          var qG = qRow.addComponent(Graphics);
          ...
          var qualities = ['流畅 (60fps)', '均衡 (HD)', '极清 (Retina)'];
          qualities.forEach((q, i) => {
            var isSel = this.selectedQuality === i;
            var btn = this.createNode("Qual_" + i, new Vec3(speedXs[i], 0, 0), qRow);
            this.ensureTransform(btn, 116, 36);
            var bg = btn.addComponent(Graphics);
        ...
        renderGameplayRulesTab(parent) {
          var rulesBox = this.createNode('RulesBox', new Vec3(0, 0, 0), parent);
          this.ensureTransform(rulesBox, 600, 320);
          var g = rulesBox.addComponent(Graphics);
          g.fillColor = this.hex('#1E293B');
          g.fillColor.a = 180;
          ...
          var rules = ['🔷  放置与接通：拖拽下方卡牌堆的水晶地砖，放置在上方棋盘对应方格，接通从起点至终点的光轨通道。', '⏱️  子弹时间：当小球行进到拐角或通路中断处时，系统自动触发极限减速，为你争取宝贵的反应时间！', '🔄  预测与擦除：点击右侧 [预览] 可预测光轨走向；放置失误或卡死时，可点击 [擦除] 重置当前手牌。', '🎁  救场与无敌：若小球不幸坠落，可通过微信好友助力瞬间复活，并获得 3 秒无敌光环冲过难关！'];
          rules.forEach((r, idx) => {
            var y = 60 - idx * 56;
            var rNode = this.createNode("Rule_" + idx, new Vec3(0, y, 0), rulesBox);
            this.ensureTransform(rNode, 550, 48);
            var rg = rNode.addComponent(Graphics);
        ...
        renderAboutTab(parent) {
          var aboutBox = this.createNode('AboutBox', new Vec3(0, 20, 0), parent);
          this.ensureTransform(aboutBox, 580, 240);
          var g = aboutBox.addComponent(Graphics);
          g.fillColor = this.hex('#1E293B');
          g.fillColor.a = 200;
          ...
          var resetBtn = this.createNode('ResetBtn', new Vec3(0, -140, 0), parent);
          this.ensureTransform(resetBtn, 260, 46);
          var rg = resetBtn.addComponent(Graphics);
